...
import { fmtEval, fmtSwing, TAG_LABEL } from '../tiny-eval'
 ...
 *
 *  Every value here is generated by tiny-eval rather than fetched, so the only
 *  untrusted thing on the line is still the model's own commentary. */
function evalChip(entry: LogEntry): string {
  ...
  const title = `${fmtEval(read)} after this move — client-side evaluation, white’s point of view`
