...
import { fmtEval, fmtSwing, TAG_LABEL, type MoveEval } from '../verdict'
...
export class Hud {
  ...
  /** The rendered line for each verdict id, so a late verdict can be dropped
   *  into a line that is already on screen without redrawing the log. */
  ...
  /** Fills in the verdict on a line that was printed before the engine had
   *  finished with it.
   *
   *  A miss is expected rather than exceptional: the log keeps a bounded number
   *  of lines, so on a long game the search can outlive the line it belongs to.
   *  There is nothing to say about that — the move has scrolled away. */
  patchVerdict(id: number, verdict: MoveEval) {
    const entry = this.entries.find((e) => e.id === id)
    if (!entry) return
    entry.eval = verdict
    const li = this.lines.get(id)
    if (!li) return
    const chip = evalChip(entry)
    if (chip) li.querySelector('b')?.insertAdjacentHTML('afterend', chip)
  }
  ...
  log(entry: LogEntry) {
    ...
    if (entry.id !== undefined) this.lines.set(entry.id, li)
    ...
    li.innerHTML = entry.detail
      ? `${label}${evalChip(entry)}<span class="detail">${escapeHtml(entry.detail)}</span>`
      : `${label}${evalChip(entry)}`
    this.logEl.appendChild(li)
    while (this.logEl.childElementCount > LOG_LIMIT) this.logEl.removeChild(this.logEl.firstChild!)
    // The map would otherwise grow for the length of the series, holding on to
    // every <li> the log has already dropped.
    for (const [key, el] of this.lines) if (!el.isConnected) this.lines.delete(key)
...
/** The verdict on a move, for the moves that earned one. Most did not, and a
 *  quiet log is what makes the loud lines mean anything.
 *
 *  Every value here comes from the engine in this browser rather than from the
 *  network, so the only untrusted thing on the line is still the model's own
 *  commentary. */
function evalChip(entry: LogEntry): string {
  const read = entry.eval
  if (!read) return ''
  const label = TAG_LABEL[read.tag]
  if (!label) return ''
  const swing = read.tag === 'mate' ? '' : ` ${fmtSwing(read.loss)}`
  const title = `${fmtEval(read)} after this move — Stockfish, white’s point of view`
  return `<span class="verdict ${read.tag}" title="${escapeHtml(title)}">${label}${escapeHtml(swing)}</span>`
}
